...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(remained) > 0:
        next_p = model.predict(remained[:batch_size])
        b = remained[:batch_size]
        for i, transition in enumerate(next_p):
            b[i].parse_step(transition)
            if len(b[i].stack) == 1 and len(b[i].buffer) == 0:
                remained.remove(b[i])
    dependencies = [parser.dependencies for parser in parses]
