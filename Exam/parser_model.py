...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        #Exam_Layer Start
        dim_exam_layer = (self.hidden_size, self.hidden_size)
        self.embed_to_hidden_weight_exam_layer = nn.Parameter(torch.empty(dim_exam_layer))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight_exam_layer)
        
        self.embed_to_hidden_bias_exam_layer = nn.Parameter(torch.empty(self.hidden_size))
        nn.init.uniform_(self.embed_to_hidden_bias_exam_layer)
        self.RelU = nn.ReLU()
        #Exam_Layer End
    ...
    def forward(self, w):
        ...
        h = self.dropout(h)
        #Exam
        exam_h = self.RelU(h @ self.embed_to_hidden_weight_exam_layer + self.embed_to_hidden_bias_exam_layer)
        #Exam
        logits = exam_h @ self.hidden_to_logits_weight + self.hidden_to_logits_bias
        ### END YOUR CODE
        return logits
