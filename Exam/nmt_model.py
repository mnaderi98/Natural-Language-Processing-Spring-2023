...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.combined_output_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        X = self.model_embeddings.source(source_padded.reshape(-1)).reshape(
            (source_padded.shape[0], source_padded.shape[1], self.embed_size))
        ...
        # Exam
        # last_hidden = torch.matmul(last_hidden[0], last_hidden[1].T)
        # print("shape : ")
        # print(last_hidden.size())
        # init_decoder_hidden = self.h_projection(last_hidden)
        # last_cell = torch.matmul(last_cell[0], last_cell[1])
        # init_decoder_cell = self.c_projection(last_cell)
        # dec_init_state = (init_decoder_hidden, init_decoder_cell)
        ### END YOUR CODE
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings.target(target_padded.reshape(-1)).reshape(
            [target_padded.size(0), batch_size, self.embed_size])
        att_projection_hidden = self.att_projection(enc_hiddens)
        for i in torch.split(Y, 1, dim=0):
            ii = torch.squeeze(i, 0)
            Y_bar = torch.cat((ii, o_prev), 1)
            dec_state, combined_output, e_t = self.step(
                Y_bar, dec_state, enc_hiddens, att_projection_hidden, enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a = torch.bmm(alpha, enc_hiddens)
        a = a.squeeze(1)

        U = torch.cat((a, dec_hidden), 1)
        # print("U = ")
        # print(U)
        #Exam
        U_prim = torch.matmul(a.T, dec_hidden)
        # print("U_prim = ")
        # print(U_prim)
        #Exam
        V = self.combined_output_projection(U_prim)
        O = self.dropout(torch.tanh(V))

        ### END YOUR CODE

        combined_output = self.dropout(torch.tanh(self.combined_output_projection(U)))
