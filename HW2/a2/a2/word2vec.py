...
def naiveSoftmaxLossAndGradient(
    ...
    # to integer overflow.
    z = np.dot(outsideVectors, centerWordVec)  # (uoT.vc)
    yo = softmax(z)  # yo = softmax(uoT.vc)
    loss = -np.log(yo[outsideWordIdx])
    h = yo.reshape((yo.shape[0], 1))
    term = np.transpose(centerWordVec.reshape((centerWordVec.shape[0], 1)))
    gradOutsideVecs = np.dot(h, term)
    gradOutsideVecs[outsideWordIdx] -= centerWordVec
    term1 = -outsideVectors[outsideWordIdx]  # -uo
    term2 = np.dot(np.transpose(outsideVectors), yo)  # U.(y-hot)T
    gradCenterVec = term1 + term2
    # END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    uo_dot_vc = np.dot(uo, centerWordVec)
    context_sigmoid = sigmoid(uo_dot_vc)
    context_J = - np.log(context_sigmoid)
    uj = outsideVectors[negSampleWordIndices, :]
    vc = centerWordVec.reshape((centerWordVec.shape[0], 1))
    not_context_sigmoid = sigmoid(-np.dot(uj, vc))
    not_context_J = -np.sum(np.log(not_context_sigmoid), axis=0)
    loss = context_J + not_context_J

    context_J_grad = - (1 - context_sigmoid) * uo
    not_context_J_grad = np.sum((1 - not_context_sigmoid) * uj, axis=0)
    gradCenterVec = context_J_grad + not_context_J_grad
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    context = (1 - not_context_sigmoid) * centerWordVec
    for i, vector in enumerate(negSampleWordIndices):
        gradOutsideVecs[vector] = gradOutsideVecs[vector] + context[i]
    gradOutsideVecs[outsideWordIdx] = (context_sigmoid - 1) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    context_num = len(outsideWords)
    for i in range(context_num):
        o = word2Ind[outsideWords[i]]
        J, J_grad_vc, J_grad_uo = word2vecLossAndGradient(
            vc, o, outsideVectors, dataset)
        gradOutsideVectors = np.add(gradOutsideVectors, J_grad_uo)
        gradCenterVecs[c] = np.add(gradCenterVecs[c], J_grad_vc)
        loss += J
    # END YOUR CODE
